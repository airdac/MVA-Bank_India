...
# Rename the Categorical feature in a concise name
```{r}
...
# Transform variables 
# First, we have an age feature (age when they applied to the loan) that is a negative number and computed as the days spent in their lifetime until the moment they apply for the loan. We want to transform it into years, for a more comprehensive purpuso.
```{r}
...
# Run the little test to see if the missing value are MCAR, MAR or MNAR.
```{r}
...
# Outlier detection 
# Once we have the PCA (1rst and 2n component), we can use Mahalanobis distance to detect outlier using with a cutoff of 95%
